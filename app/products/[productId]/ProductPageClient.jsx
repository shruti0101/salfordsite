    ...
    <>
              ...
              switch (block.type) {
                case "h2":
                  return (
                    <h2
                      key={i}
                      className="text-xl sm:text-2xl md:text-3xl font-semibold mt-8"
                    >
                      {block.text}
                    </h2>
                  );
                ...
                case "table":
                  return (
                    <div key={i} className="mt-8 w-full">
                      {/* Optional table heading */}
                      {block.title && (
                        <h3 className="text-xl sm:text-2xl font-semibold mb-5 text-black">
                          {block.title}
                        </h3>
                      )}
                        ...
                        <table className="w-full min-w-[600px] border-collapse bg-white">
                          ...
                          {Array.isArray(block.headers) &&
                            block.headers.length > 0 && (
                              <thead>
                                <tr className="bg-[#038be9] text-white">
                                  {block.headers.map((header, index) => (
                                    <th
                                      key={index}
                                      className="px-4 sm:px-6 py-4 text-left font-semibold text-sm sm:text-base border-r border-white/20 last:border-r-0"
                                    >
                                      {header}
                                    </th>
                                  ))}
                                ...
                                >
                                  {Array.isArray(row) &&
                                    row.map((cell, cellIndex) => (
                                      <td
                                        key={cellIndex}
                                        className={`px-4 sm:px-6 py-4 text-sm sm:text-base align-top border-r border-gray-200 last:border-r-0 ${
                                          cellIndex === 0
      ...
      <section className="mt-16 md:mt-20 px-4 sm:px-8 mb-16">
        <div className="text-center">
          <h2 className="text-2xl sm:text-3xl md:text-5xl font-serif font-semibold text-[#014AAB] border-b pb-2 inline-block">
            Related Items
          </h2>
        </div>
              ...
              {related.slice(0, 4).map((relatedProduct) => (
                ...
                >
                  <div className="p-6 flex flex-col flex-grow bg-[#B6E0EC]/40">
                    ...
                    <span className="block h-1 w-0 bg-gradient-to-r from-[#8AB0C2] to-[#0082C8] rounded-full mt-4 transition-all duration-300 group-hover:w-full mx-auto" />
                  </div>
                </div>
              ))}
            </div>
